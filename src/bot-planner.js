...
import { LOCAL_UNIT_PIXELS } from "./equation-engine.js";
...
export function routeToHingeEquation(route, origin, unitPixels = LOCAL_UNIT_PIXELS) {
  const points = worldRouteToLocal(route, origin, unitPixels);
...
export function routeToHarmonicEquation(route, origin, options = {}) {
  const points = worldRouteToLocal(route, origin, options.unitPixels ?? LOCAL_UNIT_PIXELS);
  if (points.length < 2) return "f(x) = 0";
...
export function buildRouteShot(routeCandidate, {
  ...
  const unit = Number.isFinite(unitPixels) && unitPixels > 0 ? unitPixels : LOCAL_UNIT_PIXELS;
  const perfectRoute = routeCandidate.route;
  const perfectEquation = family === "harmonic"
    ? routeToHarmonicEquation(perfectRoute, origin, { unitPixels: unit })
    : routeToHingeEquation(perfectRoute, origin, unit);
  ...
  // maxOffset is a caller-supplied pixel miss distance.
  const offset = severity === 0
    ? 0
    : sign * (maxOffset / unit) * severity * (0.78 + magnitudeRoll * 0.22);
  if (offset === 0) {
  ...
  const localRoute = worldRouteToLocal(perfectRoute, origin, unit);
  const endpoint = localRoute[localRoute.length - 1];
  // The perpendicular is a direction, so it stays correct computed in pixels.
  ...
  const maximumHorizontalShift = Math.max(2 / unit, Math.abs(endpoint.x) * 0.48);
  ...
  const perturbedRoute = localRouteToWorld(perturbedLocal, origin, unit);
  const equation = family === "harmonic"
    ? routeToHarmonicEquation(perturbedRoute, origin, { unitPixels: unit })
    : routeToHingeEquation(perturbedRoute, origin, unit);
...
/**
 * Routes are planned in world pixels, but the hinge and harmonic generators
 * below turn their points straight into equation coefficients. Converting to
 * the shooter's local units here is what keeps a bot's emitted equation in the
 * same units a player types, and the curve itself is unchanged: scaling both
 * axes by the same factor is a similarity transform.
 */
function worldRouteToLocal(route, origin, unitPixels = LOCAL_UNIT_PIXELS) {
  return (route ?? []).map((point) => ({
    x: (point.x - origin.x) / unitPixels,
    y: (origin.y - point.y) / unitPixels
  }));
}

function localRouteToWorld(route, origin, unitPixels = LOCAL_UNIT_PIXELS) {
  return (route ?? []).map((point) => ({
    x: origin.x + point.x * unitPixels,
    y: origin.y - point.y * unitPixels
  }));
}
...
/**
 * Parser-safe c*max(0,z), for the ReLU-style hinge basis. abs() is not
 * available to players, so |z| is built from sqrt(z^2) instead — identical
 * for every real z, and sqrt() remains an allowed function.
 */
function positivePartTerm(coefficient, expression) {
  return `${formatNumber(coefficient / 2)} * ((${expression}) + sqrt((${expression})^2))`;
}
